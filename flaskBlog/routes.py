...
def reset_request():
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for('login'))
    return render_template('reset_request.html', tilt = 'Reset Password', form = form)
...
def reset_token(token):
    ...
    if user is None:
        ...
        if form.validate_on_submit():
            ...
            db.session.commit()
            flash(f"Password has been reset for {form.username.data}!!", 'success')
            return redirect(url_for('login'))
        return render_template('reset_token.html', tilt = 'Reset Password', form = form)
